...
class RadarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 1; i <= 4; i++) {
      ...
      ringPaint.color = red.withOpacity(i == 4 ? 0.25 : 0.12);
      canvas.drawCircle(Offset(cx, cy), r, ringPaint);
    }

    final chPaint = Paint()..color = red.withOpacity(0.2)..strokeWidth = 0.8;
    ...
    final sweepPaint = Paint()
      ..shader = SweepGradient(
        ...
        colors: [Colors.transparent, red.withOpacity(0.5)],
    ...
    canvas.drawLine(
      ...
      Paint()..color = redL.withOpacity(0.85)..strokeWidth = 1.5,
    );

    canvas.drawCircle(Offset(cx, cy), 4 * pulse,
        Paint()..color = red.withOpacity(0.6)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 5));
